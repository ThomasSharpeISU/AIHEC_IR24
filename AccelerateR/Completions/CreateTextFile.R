...
#Typically I would load all packages at the beginning in the prep environment kind of area of my code. 
#For the presentation, I am going to load packages as I need them throughout to better illustrate what is being used and why.

library(stringr)
library(dplyr)
setwd('C:\\Users\\Thomas Sharpe\\Desktop\\AIHEC_IR24\\AccelerateR\\Completions') #change to where you want to operate out of

#Direct SQL Database Usage (What I more typically use)
  
    # Getting SQL Script from a prepared .sql file.
    
        #Credit to https://stackoverflow.com/questions/44853322/how-to-read-the-contents-of-an-sql-file-into-an-r-script-to-run-a-query for this, not me.
        source('readSQLScript.R')
        qCompletions <- getSQL('sqlScript.sql')
        
    #Building Connection to Database
    
        library(ROracle) #Oracle Specific
        library(DBI) # More general, works with various databases/odbc connections
        #library(odbc) #another popular option
        library(keyring) #Storing credentials in a more secure way
        
        
        #Custom function to streamline when you have multiple different databases you might connect to
        #Naturally, wont initially work for you. You would need to set up your odbc connections and what not. 
        source('C:\\Users\\Thomas Sharpe\\Desktop\\AIHEC_IR24\\AccelerateR\\Completions\\getConn.R')
        con <- dbConn('CustomServerName', key_get('UserName'), key_get('Password'))

    #Loading Data
        
        dCompletions <- dbGetQuery(con, qCompletions)
        
# Excel Load (so you can actually run the text file creation)
        
        library(readxl)
        
        #Reminder. This is simulated (fake) data
        #You will need to change the directory to where you downloaded and saved the data to
        dPrograms <- read.csv('C:\\Users\\Thomas Sharpe\\Desktop\\AIHEC_IR24\\AccelerateR\\_data\\programData.csv')
        dCompletions <- read_excel('C:\\Users\\Thomas Sharpe\\Desktop\\AIHEC_IR24\\AccelerateR\\_data\\completionData22.xlsx')
